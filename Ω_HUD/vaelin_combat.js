...
// 2. Create Stable Container (Prevents Layout Shift)
const combatContainer = dv.el("div", "", {
    attr: {
        style: "min-height: 550px; contain: layout; display: flex; flex-direction: column; gap: 10px;"
    }
});

// 3. Dynamic Armor & Speed Logic
...
const weaponRows = equippedWeapons.map(w => {
    const tags = helpers.getSafeArray(w.tags).join(" ").toLowerCase();
    let statMod = (tags.includes("ranged") || tags.includes("finesse")) ? helpers.getMod(stats.dex) : helpers.getMod(stats.str);
    ...
    let icon = tags.includes("ranged") ? "🏹" : "⚔️";
...
// 5. Build Spell Table Data
const spellRows = data.magic.spells_and_abilities
    .filter(s => helpers.getSafeArray(s.tags).some(tag => tag.toLowerCase() === "combat"))
    .map(s => {
        ...
        if (notesStr.includes("save")) atkOrDc = `<b>DC ${spellDc}</b>`;
        
        return `<tr><td>✨ ${s.name}</td><td><center>${atkOrDc}</center></td><td>Varies</td><td>Magic</td></tr>`;
    }).join('');

// 6. Assemble Final HTML Output
const weaponList = "option(None),option(Dagger),option(Shortsword),option(Rapier),option(Shortbow)";
...
<div class="callout" data-callout="abstract" style="margin-top: 10px;">
    <div class="callout-title">Combat Controls</div>
    <div class="callout-content">
        Armor: ${helpers.createInput("inlineSelect(option(Unarmored),option(Studded Leather))", "core", "combat_base.equipped_armor")}<br>
        Masteries: ${helpers.createInput(`inlineSelect(${weaponList})`, "core", "combat_base.active_masteries.weapon_1")} | 
        ${helpers.createInput(`inlineSelect(${weaponList})`, "core", "combat_base.active_masteries.weapon_2")}
    </div>
</div>

<h3>⚔️ Weapon Attacks</h3>
<table style="width: 100%;">
    <thead><tr><th>Weapon</th><th><center>Atk</center></th><th>Damage</th><th>Range</th><th>Mastery</th></tr></thead>
    <tbody>${weaponRows}</tbody>
</table>

${spellRows.length > 0 ? `<h3>🪄 Combat Spells</h3><table style="width: 100%;">
    <thead><tr><th>Spell</th><th><center>Atk/DC</center></th><th>Damage</th><th>Type</th></tr></thead>
